-- A download link can now point at a picture inside a Google Doc, which is a
-- fourth kind. `kind` names its kinds in a CHECK constraint, and SQLite cannot
-- widen one in place, so the table is rebuilt: new table, copy, drop, rename.
--
-- Everything 0001 gave the table is written out again — STRICT, the text
-- primary key, both foreign keys with ON DELETE CASCADE, the use counter's
-- check and its default, and the index on expires_at, which the drop takes
-- with it. Nothing references `links`, so nothing else can be left pointing at
